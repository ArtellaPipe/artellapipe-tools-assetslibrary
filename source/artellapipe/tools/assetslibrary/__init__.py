...
import sentry_sdk
sentry_sdk.init("https://6ce3277bc9d646029e1062bdf7e56f24@sentry.io/1764122")
...
class AssetsLibrary(importer.Importer, object):
    def __init__(self):
        super(AssetsLibrary, self).__init__(module_name='artellapipe.tools.assetslibrary')
...
def run(project, do_reload=False):
    ...
    win = assetslibrary.run(project=project)
    return win
...
def get_logging_level():
    ...
    if os.environ.get('ARTELLAPIPE_TOOLS_ASSETSLIBRARY_LOG_LEVEL', None):
        return os.environ.get('ARTELLAPIPE_TOOLS_ASSETSLIBRARY_LOG_LEVEL')

    return os.environ.get('ARTELLAPIPE_TOOLS_ASSETSLIBRARY_LOG_LEVEL', 'WARNING')
